...
const INTENT_PATTERNS = [
  {
    intent: "deploy",
    re: /\b(deploy|open|add liquidity|lp into|invest in)\b/i,
  },
  {
    intent: "close",
    re: /\b(close|exit|withdraw|remove liquidity|shut down)\b/i,
  },
  ...
  {
    intent: "config",
    re: /\b(config|setting|threshold|update|set |change)\b/i,
  },
  ...
  {
    intent: "screen",
    re: /\b(screen|candidate|find pool|search|research|token)\b/i,
  },
...
export async function agentLoop(
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      const toolChoice =
        step === 0 && agentType === "GENERAL" && ACTION_INTENTS.test(goal)
          ? "required"
          : "auto";
      ...
      if (!response.choices?.length) {
        log(
          "error",
          `Bad API response: ${JSON.stringify(response).slice(0, 200)}`,
        );
        throw new Error(
          ...
          if (tc.function?.arguments) {
            try {
              ...
              try {
                ...
                log(
                  "warn",
                  `Repaired malformed JSON args for ${tc.function.name}`,
                );
              } catch {
                tc.function.arguments = "{}";
                log(
                  "error",
                  `Could not repair JSON args for ${tc.function.name} — cleared to {}`,
                );
              }
            }
          }
        ...
        msg.tool_calls.map(async (toolCall) => {
          ...
          try {
            ...
            try {
              ...
              log(
                "error",
                `Failed to parse args for ${functionName}: ${parseError.message}`,
              );
              functionArgs = {};
            }
          }
